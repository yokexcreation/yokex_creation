...
// Load events from Firebase on start
useEffect(()=>{
...
}catch(err){showToast("⚠️ Failed to upload "+file.name);}
}
if(uploaded.length>0){
try{
const ref=doc(db,"events",activeId);
await updateDoc(ref,{photos:arrayUnion(...uploaded)});
...
<div key={i} className="ph" style={S.photoCard} onClick={()=>setLightbox({photo:p,ev:activeEv})}>
...
<div key={i} className="ph" style={S.photoCard} onClick={()=>setLightbox({photo:p,ev:guestEv})}>
<img src={p.thumb||p.url} style={S.photoImg}/>
...
<div style={{fontSize:12,color:"#888",marginBottom:24,lineHeight:1.6}}>Print this QR and place at venue. Guests scan to instantly view & download all photos permanently!</div>
